...
> **不要**再对大文件做无条件全文 Read。
> **第二跳**：锚点 → `offset`/`limit` 换算查 `ctx/hot-files.md`（热文件与大文件，人/agent 可直读；
> 由 `pnpm run ctx:build` 生成）。查不到的文件（非热非大）再查 `ctx/index.json`（全量，机器读）。
...
2. 到 `ctx/hot-files.md` 查该锚点的 `offset`/`limit`（行内格式 `锚点=offset+limit`，已算好）。
   - 该文件**只收录热文件与大文件**；查不到 → 退到 `ctx/index.json` 查 `startLine` / `endLine`。
     （`ctx/index.json` 是**全量**索引，体积大、机器读更划算，**不要**整读它。）
