...
> **常驻总量**（AGENTS.md + my-rules/* + ctx/hot-files.md + ctx/ROUTES.md，每次会话固定开销）= **13225** 估算 tokens（观察哨软阈值 ≤ 13500）：单文件上限各自为政时总量仍可漂移，本行仅观察提示、不阻断；硬阻断只挂各单文件门。

## 2. Top 20 大文件（估算 tokens）

| # | 文件 | tokens | 行数 | tier |
|---:|---|---:|---:|:--:|
| 1 | `production/qa/beads/g4-regression-report.md` | 44524 | 916 | normal |
| 2 | `production/TASKS-DETAIL.md` | 18586 | 355 | normal |
| 3 | `production/qa/beads/test-cases.md` | 17894 | 375 | hot |
...
| 10 | `games/beads/design/audio/audio-spec.md` | 10410 | 293 | normal |
...
| 18 | `games/beads/cocos/extensions/cocos-mcp-server/README.pt.md` | 8518 | 525 | normal |
| 19 | `games/breakout/cocos/extensions/cocos-mcp-server/README.pt.md` | 8518 | 525 | normal |
...
| 文件 | tokens | 处置 |
|---|---:|:--:|
| `production/qa/beads/g4-regression-report.md` | 44524 | 见 `ctx/budget-exempt.json`（无豁免则 ❌ 需报告主理人） |
| `production/TASKS-DETAIL.md` | 18586 | 见 `ctx/budget-exempt.json`（无豁免则 ❌ 需报告主理人） |
| `production/qa/beads/test-cases.md` | 17894 | 见 `ctx/budget-exempt.json`（无豁免则 ❌ 需报告主理人） |
...
| `games/beads/design/audio/audio-spec.md` | 10410 | 见 `ctx/budget-exempt.json`（无豁免则 ❌ 需报告主理人） |
