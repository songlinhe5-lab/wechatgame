...
> **常驻总量**（AGENTS.md + my-rules/* + ctx/hot-files.md + ctx/ROUTES.md，每次会话固定开销）= **14168** 估算 tokens（观察哨软阈值 ≤ 13500，⚠️ 已超——请评估瘦身）：单文件上限各自为政时总量仍可漂移，本行仅观察提示、不阻断；硬阻断只挂各单文件门。
...
| 2 | `games/beads/art/assets-spec.md` | 70052 | 1438 | normal |
| 3 | `production/TASKS-DETAIL.md` | 35564 | 538 | normal |
...
| `games/beads/art/assets-spec.md` | 70052 | 见 `ctx/budget-exempt.json`（无豁免则 ❌ 需报告主理人） |
| `production/TASKS-DETAIL.md` | 35564 | 见 `ctx/budget-exempt.json`（无豁免则 ❌ 需报告主理人） |
