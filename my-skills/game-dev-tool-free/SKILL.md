...
description: 通用游戏开发助手免费版（本仓库存档保留）。仅当用户在对话中明确点名「game-dev-tool-free」或「游戏开发助手免费版」时才使用；本 monorepo 的概念/GDD/UX/架构/Epic/QA/发布/音频任务一律优先使用 wxgame-* 系列 skill。不要因「全流程」「游戏开发」「GDD」「测试」「发布」等泛化关键词自动触发本 skill。
---
# 游戏开发助手 (免费版)
