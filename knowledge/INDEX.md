...
| K-057 | 流程 | process | 「立项单」与「ADR 状态行」都不会被实施它们的另一张单自动收口，核销时要成对扫 | WXG-T-166 | 2026-09-19 | 0 | active |
| K-058 | 工具链 | toolchain | 「已写进 .gitignore」不等于「已排除出索引产物」——枚举型走盘生成器只认自己的硬编码跳过集 | WXG-T-176 | 2026-09-19 | 0 | active |
| K-059 | 流程 | process | 用「甲/乙/丙」转述既有候选清单时必须带正文锚——字母会换义 | WXG-T-167 | 2026-09-19 | 0 | active |
| K-060 | 测试 | testing | 变异自检里断言「方向」选错会留下伪绿：被测标量在变异下「冻结在旧非零值」而非归零时，存在性断言无判别力 | WXG-T-128 | 2026-09-19 | 0 | active |
| K-061 | 测试 | testing | `noAssemble` 下手工铺盘 ≠ 装配：子系统内部索引未建，表现为「点名了但不动手」，极易误判为业务缺陷 | WXG-T-128 | 2026-09-19 | 0 | active |
| K-062 | 测试 | testing | 事件驱动宿主的输入位移必须进单测：prev 在帧首结算会把帧间到达的位移吞成 0 | WXG-T-178 | 2026-09-19 | 0 | active |
| K-063 | 判据 | criteria | 会「静默转移」的锚不能当距离排序基准：消费序基准与展示坐标必须分离 | WXG-T-183 | 2026-09-20 | 0 | active |
| K-064 | 判据 | criteria | 跨工具链的「草案自报元数据」必须被消费端复核归一：手工合规草案的单测永远抓不到生产端的字段说谎 | WXG-T-179 | 2026-09-20 | 0 | active |
| K-065 | 环境 | environment | 国内云 VPS 上 Docker 部署必踩的两道坎：镜像仓库不可达 + 云侧安全组（与主机防火墙是两层） | WXG-T-179 | 2026-09-20 | 0 | active |
| K-066 | 工具链 | toolchain | 长期分支间用 squash 同步 ⇒ 下次同步「全域冲突」，且 3-way 会静默产出重复定义 | WXG-T-179 | 2026-09-20 | 0 | active |
...
| K-081 | 工具链 | toolchain | 台账详情节的节标题只认「整行形状」：`## WXG-T-0NN` 后带文字 = 节静默丢失 | WXG-T-0 | 2026-09-26 | 0 | active |
| K-082 | 测试 | testing | 珠体族视觉改动 ⇒ 封箱对照腿必然红是预期行为，唯一合法通道 = 复评归因（禁自更新基准） | WXG-T-214 | 2026-09-26 | 0 | active |
| K-083 | 测试 | testing | 「面积占比 ≥X%」是代理指标不是感知标准；感知判据 = 对比度（3:1）+ 样本视角（≥1°），且面积账必须含圆角修正 | WXG-T-214 | 2026-09-26 | 0 | active |
<!-- kb:active:end -->
