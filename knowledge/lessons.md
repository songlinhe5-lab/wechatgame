...
- **[流程][K-039] 报绿之后又动了源码：交付自述必须与最终状态同一时点**（来源 WXG-T-096 复核轮，2026-09-15）
  ...
  规避：① 每次改 `packages/framework/src/**` 后固定「`framework:sync` → `verify`」连跑，任何写进文档/台账的数字（例数、PASS 数）必须在**最后一次改动之后**重取；② 建议 pre-commit 增条件步：暂存区含 `packages/framework/src/**` 才跑 `sync:check`（待主理人拍板，勿为凑绿放宽）；③ 指认「工具有 bug」前先读该工具源码并拿 git HEAD 两侧取证。
