...
- 上位来源：`gdd/meta-ui.md` v0.3（Album→Plate→Cell-Level 三层对象）· `ADR-0004`（行字符串数据形态）· `design/levels/levels-spec.md` · `systems-index §3.3`（`GRID_MAX=32`，v1.45）
...
|---|---|---|---|---|
| **2048×2048** | **32** | **64×64** | 64 > 32 ⇒ Plate 切块（k=ceil(64/32)=2 → `[32,32]×[32,32]`，**零残余**） | **2×2 = 4 宫 32×32，一张完整组合图 4 关** |
| 2048×2048 | 64 | 32×32 | ≤32 single | 1 关 32×32（全错位超 taps 预算，实际走 swaps） |
| **1024×1024** | **32** | **32×32** | ≤32 single | 1 关 = **最大单图档**（与 2048@64px 同款盘面） |
| 1024×1024 | 64 | 16×16 | ≤32 single | 1 关（触控最佳档，ADR-0018） |

- **`GRID_MAX` = 单图/Plate 分界线 = 切块阈值**：语义不变，**取值已由 50 改为 32**（§3.3 **v1.45**，用户 2026-09-22 拍板「单图上限 32 个珠子宽度，超过就拆组合图」）。⚠️ **本条原裁定（v0.6/v0.7：「GRID_MAX=50 不改；用户此前『暂不考虑 GRID_MAX』的预设 ⇒ 取消，无需变更单」已被推翻**——分界线本身改了，且走了 §3.3 变更单 v1.45。当时反对提高上限的理由（64 会把 64×64 变成单图、触控不可达）**仍成立**，只是方向相反：32 把 40×40 这类「能导入但不可玩」的盘也归给 Plate。新依据：32 与每颗像素标准 32px 对齐 ⇒ 1024 = 最大单图、2048 = 64×64 → 2×2×32×32 零残余。
...
| v0.8 | 2026-09-22 | **切块阈值 `GRID_MAX` 50→32（§3.3 v1.45；推翻本文 v0.6/v0.7 的「GRID_MAX 不改」裁定）**。用户拍板：「单图上限 32 个珠子宽度，另外如果超过 32 珠子宽高，studio 需要拆成组合图」。落码：`level-slice.mjs` 默认 `gridMax: 32`（旧 50 阀单测保留，证明算法与阈值解耦）、新增 32 阀用例（64→[32,32] 零残余 / 33→[16,17] / 默认参数即 32）共 8/8 绿；`server.mjs` `isPlate >32`、`sliceBoard({gridMax:32})`、`importBlockers` >32 文案与「>32 不是 blocker」防误杀注记；前端 `SINGLE_MAX=32` 提为常量（提示语「超单图上限 32 ⇒ 自动拆组合图」）。§0.3 档位表同步（k=ceil(64/32)=2）。**零新常量、零包体增量**；BOOT（`levels.ts`）纯引 `GRID_MAX_*` 自动跟随，但**手工写的 >32 关卡 JSON 现被 BOOT 拒收**（已入 v1.45 负面后果）。 | WXG-T-203，用户 2026-09-22 拍板 |
