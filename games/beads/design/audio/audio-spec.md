...
- 项目：`games/beads`（拼豆填色消除）· 版本 v1.0 · 任务号 WXG-T-083（GAP-05：音频实质为零）
...
| `sfx_urgent_beat` | 正弦低频双击（心跳「lub-dub」） | 两拍内包络 | 低通 | 单拍 ≤1000 ms 周期内 | `[TODO]` |
| `sfx_tray_full` | 三角（轻提示，中性无威胁） | A 短 → D 短 | 低通 | 500 ms 视觉周期内 1 次 | `[TODO]` |
| `sfx_stage` | 噪声 + 带通扫频（「唰」，双段出/入） | 两段镜像 | 带通扫频 | 250+250 ms | `[TODO]` |
| `sfx_clear` | 琶音序列（`sfx_powerup` 同族音色，音数 `[TODO]`） | 逐音 A 极短 → D 中 | 高通 | 800 ms | `[TODO]` |
| `sfx_star` | 上行「叮」（逐颗 3 次，音高递增表 `[TODO]`） | A 极短 → D 中 | 高通 | 150 ms ×3 | `[TODO]` |
| `sfx_panel_in/out` | 短噪 + 低通扫（抽屉「唰」，入/出镜像） | A 短 → D 短 | 低通扫频 | 200 / 150 ms | `[TODO]` |
| `sfx_revive_ok` | 正弦上滑「叮」（与 `sfx_reject` 语义相反，明亮） | A 极短 → D 中 | 高通 | ≤400 ms 反馈红线内 | `[TODO]` |
| `sfx_ui_tap` | 极短正弦/三角点音 | A 极短 → D 极短 | 低通 | `[TODO]`（§5 无行，Q-A05-1） | `[TODO]` |
| `bgm_main` | **序列化音符表**（确定性序列，非随机生成）+ 软音色 pad 层（可选） | 循环乐句，节拍 `[TODO]` | 低通（垫底不抢前景） | 循环 30–60 s（建议区间） | `[TODO]` |
...
| T2 | `bgm_main` 循环时长、循环点位置、BPM、调性、乐句结构 | 合成路线：我出音符序列常量表（波次 4 与工程同批）；回退采样：`indie-game-ost-pack` 交付时附循环点说明 |
| T3 | 19 条 clip 的合成配方数值（基频 Hz / attack-decay ms / 增益 / 滤波截止） | `[B]` 可听 + `[P]` 人耳（A05-26）后定档；总时长上限已由 §5 冻结 |
| T4 | `sfx_ui_tap` 时长（§5 无行） | 待 Q-A05-1 裁决（ux-spec §5 增行）后取值 |
...
- 本篇 27 条判据（`audio-events.md` §4）= T-084 QA 用例的直接输入，按道次精确分账：**纯 `[N]` 16 条**（今天可落 vitest）+ **含 `[N]` 的混合 6 条**（A05-04/09/13/15/16/21：入队与同帧部分今天可断言，时长/听感部分挂起）+ **`[C]` 1 条**（A05-25，今天可对构建产物断言）+ **完全挂起 4 条**（A05-03 `[B][P]` / A05-22 `[B][R]` / A05-26 `[P]` / A05-27 `[R]`）。挂起项必须标注阻塞源，**不得记为已验收**。
