...
> **状态列推进（素材路线，v1.52）**：19 条 SFX 的**素材版**（`jsfxr` 离线生成 → mp3 → `src/config/audioAssets.ts`）
> 已经用户 2026-09-23 于浏览器 harness 逐条试听判为「**效果可以**」⇒ 这 19 条的状态由「已做」推进为
> **`已验收`（试听道次 = `[B]`，范围 = web/harness）**。⚠ 两点没被这次试听覆盖，不得顺手勾掉：
> ① **weapp 真机**的 `decodeAudioData` 能力未取证（`[R]`）⇒ 手机上可能仍播合成音；
> ② **相对响度**（BGM vs SFX 的 dB 档）仍 `[TODO]`，试听判的是音色，不是电平。
> 试听样带可由 `node my-skills/wxgame-audio-gen/scripts/gen-sfx.mjs` 以字节级相同产物复现（已 md5 验），
> 故样带不入库。
