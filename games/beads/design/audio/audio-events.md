...
> **状态列更正（2026-09-23 夜，浏览器实测后）**：本条先前写「19 条 SFX **素材版**已经用户于浏览器 harness 逐条试听判为效果可以 ⇒ 推进为 `已验收`」——**该表述不成立，现予更正**。
> 实测证据：harness console 出现 `[audio] decodeAudioData promise 失败 ⇒ 回退合成`，后端 `_assetBuffers.size = 0`
> ⇒ 游戏内当时播放的仍是 **notes 合成音**，素材版从未在游戏内响过。用户原话「`temp/sfx-clips/` 那 19 条效果可以」
> 判的是**离线样带文件**（`[B]` 道次成立，但对象是文件不是游戏内通路）。
> 根因与修复：`base64Bytes()` 用 `subarray` 返回**超额分配 buffer 的视图**，而 `decodeAudioData` 按 spec 收 **ArrayBuffer**
> （尾部垃圾 ⇒ "Unable to decode"）；已改 `slice` 精确拷贝 + 递 `.buffer`，并把失败原因打进入口日志。
> 修复后实测：`_assetBuffers` = **Map(19)**、零 `[audio]` 告警。
> ⇒ 因此本表状态列口径修正为：**样带文件 `已验收`（离线试听，音色可用）；游戏内素材版 = `已做`，重听待用户确认**（音色与相对响度两格均未取证）。
...
| A05-25 | 全表·包体（v1.52 二改） | **v1.51 反转**（原断言「音频文件数 = 0 / 0 KB」随采样改判作废）：新断言 = ① **SFX 侧仍零文件**（合成路线不变）② BGM 文件数 ≤ **3**、总大小 ≤ **1536 KB**（3 × 40 s × 96 kbps mono ≈ 1.44 MB）③ 产物总主包 ≤ `systems-index §3.9` 内部目标（现 4096 KB）④ 格式限 **mp3 / m4a / wav**（iOS 微信不支持 ogg）。<br>**v1.52 二改**：目标态改为「**SFX 亦素材文件**」——SFX 侧实测 base64 合计 **60.5 KB**（19 条，进主包 JS 源码）；⚠ **消费者未接入前运行时仍是合成**，现**已接线**（方案 A：`AudioVoice.asset` + `SynthAudioBackend` 在 unlock 期 `decodeAudioData`，失败回退合成）⇒ 本条改为：主包 JS 内含 base64 资产 **60.5 KB**（路径 `games/beads/src/config/audioAssets.ts`），`cocos/build/*` 内**独立音频文件数仍为 0**（数据在 JS 里，不是文件）；weapp 真机解码能力 `[R]` 待验，验收前不得声称"素材已可闻"| `[C]` |
