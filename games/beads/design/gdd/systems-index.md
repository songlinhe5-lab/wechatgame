...
| S4 | 供料与槽位托盘 | `gdd/tray-spawner.md` | P0 | **v1.22：定时供料已关停（案 A）**——托盘纯缓冲：容量管理、取回入槽、扩展行（**v1.23：基线容量 12→6**；**v1.30：24→12（1 行×12）**，§3.4） |
...
| 常量 | 值 | 说明 |
|---|---|---|
| `TRAY_BASE_SLOTS` | **12** | **v1.30 冻结变更 24→12**（WXG-T-168，用户 2026-09-18 真机体检裁定「版面没有那么多槽 ⇒ 先开放一行 12 个槽，扩容再开放第二行」）：固定 **1 行 × 12**。**推翻 v1.24（WXG-T-141，用户 2026-09-17 拍板 6→24）的取值** —— v1.24「24 槽 = 从容批量取回」论述作废。**诚实记录（负面后果）**：① 空间压力回升，且与同批 WXG-T-168 落码的**部分收纳**（空槽不足只收距锚最近 N 颗）叠加 ⇒ **部分收纳触发频率显著上升**，属用户明示取舍（扩容即解压阀）；② 若后续 playtest 显示「频繁部分收纳」损害手感，本值是最优先回调旋钮（扩回 24 只需改本行 + `tuning.ts`）。几何常量 `TRAY_COLS`/`TRAY_SLOT` 不动；面板高随行数派生（`trayLayout(rows)`，1 行 ⇒ 72），仍**贴带顶锚定**（v1.20 规则、零特判）⇒ 带上沿以下的富余区域为扩展行预留位（未扩容时不画槽） |
| `TRAY_EXPAND_SLOTS` | **12** | **v1.30 冻结变更 24→12**（同上）：**+1 行 × 12 = 12 槽**，扩展后共 24 槽（2 行）；激励视频解锁、**MVP 无广告调用**（badge 占位）、本关内有效、重开重置（A1 语义不变）。**扩展行沿用虚线槽语言**；WXG-T-168 同批修 `dashed = row > 0` 误判（旧式把**基础第 2 行**也画成虚线 ⇒ 玩家误读为「未开放」） |
| `TRAY_COLS` | 12 | 每行槽数 |
...
| `DEMO_LEVEL_COUNT` | 9 | demo 关卡数（初版用户拍板 5–10 区间取 8；**v1.30** beads-studio 一键入关转正第 9 关，随包发布，区间内递增） |
