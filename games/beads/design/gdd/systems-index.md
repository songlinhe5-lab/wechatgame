...
- 项目：`games/beads`（拼豆错位归位）· 版本 v1.23 · 任务号 WXG-T-138（**v1.23 = 「大胆重制」裁定落盘**：托盘 12→6 槽 / 时间按 k 定价 / 星级主题解锁语义 / 冲刺 k 爬梯 / cycleProfile 环长分布 / E4 幽灵符号开关转正，提案正本 `design/proposals/v1.23-bold-remake.md`；v1.22 及以前见 `systems-index-changelog.md`）
...
| 常量 | 值 | 说明 |
|---|---|---|
| `TRAY_BASE_SLOTS` | 6 | **v1.23 冻结变更 12→6**（WXG-T-138，提案 `proposals/v1.23-bold-remake.md` Q9，用户 2026-09-16 拍板）：容量是**手感旋钮**而非难度闸门——托盘 2 槽即可滚窗解任意置换（诚实修正，提案 §5）；小托盘强制滚窗操作、珠子高频换血，凑近「手作工作台」触感（支柱 2 v1.23 表述）。谜题难度全押 k 与环长分布（§3.13）。几何常量 `TRAY_COLS`/`TRAY_SLOT` 不动，面板视觉收窄归 WXG-T-131 |
| `TRAY_EXPAND_SLOTS` | 6 | **v1.23 冻结变更 12→6**（同上；对 concept 附录 A1 的二次变更，已登记 changelog）：扩展后共 12 槽；激励视频解锁、本关内有效、重开重置（A1 语义不变） |
| `TRAY_COLS` | 12 | 每行槽数 |
| `TRAY_SLOT` | 48 | 槽位边长（内凹空槽） |
| `TRAY_GAP` | 6 | 槽间距 |
| 托盘槽定位 | 水平居中；垂直**贴上沿** | 12×54−6 = 642 ≤ 可用宽 690。垂直锚定 v1.20 由「带内居中」改「贴带上沿」（面板顶边 = `TRAY_BAND.yMax`）；**带加高不能解决重叠**：面板若仍居中，每侧净空 = (带高−116)/2 需 ≥ 88 ⇒ 带高 ≥ 292 ⇒ 上沿撞 12 行关最底行格热区（481） ✗ |
