...
| 常量 | 值 | 说明 |
|---|---|---|
| `LEVEL_TIME_DEFAULT` | 300 s | 关卡默认倒计时（**兜底值**：关卡 JSON 无 `time` 覆盖时用）；关卡可覆盖，合法区间 **[120, LEVEL_TIME_MAX]**（v1.23 由 [180,420] 放宽下沿，WXG-T-138；上沿 420 → **2500** 见 v1.47） |
...
| 规模闸（实测硬拦）· **v1.41 新增** | `taps × SEC_PER_TAP ≤ LEVEL_TIME_MAX` ∧ bot 通关 | 用户 2026-09-21 拍板「生成期拒产 + 入关期拒收」：`beads-gen` 打印完整报告后 `exit 4`；Studio `ingestLevel` 回 422（bot 不可用 ⇒ 500，**不回落静态估算**）。判据只用**真值**（真引擎实测点击数，`tools/scripts/beads-bot.ts`）。**为何不设 M / B_med 静态闸**（曾拟 `M ≤ 280 ∧ B_med ≥ 8`，已入库 10 关实测**证伪并删除**）：甜心 B_med=6 而 44 taps 轻松通关、猫咪脸 B_med 同为 6 而 134 taps 超预算 ⇒ 静态量无分辨力，用作闸纯误伤 3 关；bot 单盘 0.3s 也不需早期剪枝。正本 `levels-spec §5.0.0`。**v1.47 顶值变更**：本闸等价于「点击硬顶 = `LEVEL_TIME_MAX` ÷ 3.6」，旧 420 ⇒ **116 击**，而全错位盘实测 `taps ≈ 0.65 × 错位珠数`（逐动作取证：31×31 大色块盘 M=828 / 同色相邻率 0.843 / 540 击，其中 50% 是「选取」点击、执行击均值仅 3.07 颗）⇒ 旧顶系统性误杀大色块全错位盘。用户 2026-09-22 拍板顶值改 **2500s（≈694 击）作开发期验证档**。**历史债撤销**：L7 猫咪脸 134 taps ⇒ 482s 在旧顶下「不可入关」，v1.47 后 482 ≤ 2500 ⇒ **不再触闸**，该挂账撤销（其 time=403s 仍为 playtest 校准值，在合法区间内）。**新增挂账**：难度分 `D = 100 × T / LEVEL_TIME_MAX` 分母随顶值漂移（D 不可跨档比较）；大盘 3★ 与失败页/续时激励位开张率待 playtest 定档。 |
