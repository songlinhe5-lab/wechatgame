...
| `GRID_MAX_COLS` | **29** | 单关列数上限。**v1.34（WXG-T-180）13 → 29**：对齐 5mm Midi **标准方形盘 29×29 = 841 颗**（用户盘面调研 + MVP 目标）。⚠️ **连带**：29×52−2 = **1506 > 设计宽 750** ⇒ 大盘**不再完整可见**，须靠缩放（ADR-0015 丁-3 已落码）；旧口径「≤ 可用宽 690」**随之作废**（仅对 ≤13 列成立，`tests/tuning.test.ts` 已按「屏内容得下的最大盘」迁移 + 新增「29×29 溢出」的诚实断言） |
