...
- 项目：`games/beads`（拼豆错位归位）· 版本 v1.22 · 任务号 WXG-T-130（**版本号与 `systems-index-changelog.md` 末行同步维护**；旧版此行停在 v1.12 系 WXG-T-098 核对时发现的漂移）
...
| S4 | 供料与槽位托盘 | `gdd/tray-spawner.md` | P0 | **v1.22：定时供料已关停（案 A）**——托盘纯缓冲：容量管理、取回入槽、扩展行 |
...
| `EMPTY_GHOST_ALPHA` | 0.32 | **⛔ v1.22 作废（E4 幽灵符号移除）**（WXG-T-130，用户裁定原文「暂时不需要支持色盲玩家，我需要的是珠子和托盘、棋盘都有质感」= **可访问性降级裁定·用户签字 2026-09-16**）：空槽不再绘制幽灵符号 ⇒ α 值无绘制对象。**可恢复路径**：E4 绘制逻辑保留为**关闭的开关/死路径**（不删码），重开零成本（改回本常量 + 开关置开）。降级范围与登记正本 = art `accessibility.md` A2b/A3 |
