...
- 项目：`games/beads` · 版本 **v2.3** · 任务号 **WXG-T-168**（v2.3 = **取回落槽改「玩家点槽定落位 + 整组连续相邻」+ 容量不足改「部分收纳·距锚就近优先」**，用户 2026-09-18 真机复测后**推翻 v2.1 裁定①（自动归类）**；v2.2 = **盘侧组选改完整连通块（8 向不限步）+ board 锚直填放开距离**，用户 2026-09-18 真机裁定，覆盖 T-157 ①②③，任务号 WXG-T-162；v2.1 = **托盘侧组归位（同色全组批量填充，8 向不限步、部分填充）+ 取回落槽自动归类**，用户 2026-09-18 四项裁定；v2.0 = 「错位归位」状态机改写；v1.0 见 §9）
...
- **组语义（v2.2 · WXG-T-162 用户真机裁定④⑤，覆盖 T-157 ①）**：选中锚 ⇒ 组 = 锚起 **8 向（3×3 含对角）连通、同色错位珠的完整连通块，不限步数**（「找相邻直到找不到，所有找到的珠子一起抬起」）；就位珠/锁定格/空格/异色错位珠均**阻断传播**（隔珠同色 ⇒ 不同组）。**board 锚直填**（`input-control §2.1` 5b，T-157 立、v2.2 裁定③放开距离）：点底色 = 组色的 `empty` 格，**任意距离**（旧切比雪夫 ≤2 门已删）⇒ 取组内离目标最近者（平局行主序）retrieve+fill 同帧两写直接归位（搬移语义，源格空出）；组保持逐颗续填、锚珠被填 ⇒ 锚静默转移到剩余组首、组空锚清；归位达成零错位 ⇒ cleared-priority 照常。
...
2. 托盘无空槽 → **满槽禁取珠**（§3.13）：拒绝，极轻非惩罚反馈、**零事件、零状态写**；**整组收进口径（v2.3 / WXG-T-168，覆盖旧「`free` 槽数 ≥ 组大小，不足 ⇒ 整组拒」）**：可收数 = `min(组大小, 从落槽起点起的连续空槽数)` —— **有多少空槽就收多少颗**，所收 = 组珠中**离点击位置（锚珠）最近者**；余珠留格且**锚改指剩余首颗**（可续点，锚变更不发事件）。起点零连续空槽 ⇒ 与满槽同口径拒绝。
