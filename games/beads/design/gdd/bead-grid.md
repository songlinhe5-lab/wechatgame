...
- 项目：`games/beads` · 版本 v2.2 · 任务号 WXG-T-162（v2.2 = **盘侧组选改完整连通块（8 向不限步）+ board 锚直填放开距离**，用户 2026-09-18 真机裁定，覆盖 T-157 ①②③；v2.1 = **托盘侧组归位（同色全组批量填充，8 向不限步、部分填充）+ 取回落槽自动归类**，用户 2026-09-18 四项裁定；v2.0 = 「错位归位」状态机改写；v1.0 见 §9）
