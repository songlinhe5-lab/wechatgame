...
2. 托盘无空槽 → **满槽禁取珠**（§3.13）：拒绝，极轻非惩罚反馈、**零事件、零状态写**；**整组收进口径（v2.3 / WXG-T-168，覆盖旧「`free` 槽数 ≥ 组大小，不足 ⇒ 整组拒」）**：可收数 = `min(组大小, 从落槽起点起的连续空槽数)` —— **有多少空槽就收多少颗**，所收 = 组珠**消费序的前 `count` 颗**（v2.5：与 §2.2 直填共用同一份剥皮序，拾取时一次算好；~~v2.3/v2.4「距选豆点最近者」几何序~~ ⛔ 作废）；余珠留格且**头珠改指剩余序首**（可续点，锚变更不发事件）。起点零连续空槽 ⇒ 与满槽同口径拒绝。
