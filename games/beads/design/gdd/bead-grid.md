...
- 关卡以**行字符串数组**描述图案，字符集 = `BEAD_CHARSET`（来源 §3.2）：`.`=空位（待填）、`x`=锁定格（不可填、不计完成）、`1-9`+`A`=色板索引 1–10（索引对应 `BEAD_PALETTE` 顺序，`A` 即第 10 色）。
...
- 最后一颗可填格与失败同帧：S1 按 cleared 优先裁决（core-loop §6），S3 照常广播 placed。

**并发 / 竞态**
- 同一帧多次落子请求：**串行化**——S3 逐条裁决，第一条生效后第二条的目标格若已被占则走"忽略"分支；禁止批量原子合并。
- 落子请求与 `level:cleared` 广播同帧：cleared 后到达的请求一律忽略（状态机已离开 PLAYING）。
