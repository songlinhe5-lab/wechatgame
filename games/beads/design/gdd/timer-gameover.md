...
- 项目：`games/beads` · 版本 v1.2 · 任务号 WXG-T-009（续时语义 WXG-T-057）
...
**并发 / 竞态**
- 暂停请求与归零：判定基准为**帧内序 = 输入 → 连击窗 → 供料 → 计时**（`pause-settings.md` §6，2026-09-12 裁定，WXG-T-030 实现确认），齿轮属输入、先于计时处理 ——
...
6. 失败重试后重置逐一断言：倒计时回满 / 图案全空（locked 保持）/ 托盘全空 / 扩展回基线 / 道具免费次数回 `POWERUP_FREE_USES` / `reviveBonusSec=0` 且 `revived=false`。
