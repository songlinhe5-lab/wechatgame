// 拼豆图案生成原型（WXG-T-179 前置 spike，仅用于「用图验证」，非生产码）。
//
// 管线：源图 → 量化到拼豆色板（1..8）→ 背景去格(void) → **配色平衡**(任一色 ≤½)
...
const cols = Math.max(1, parseInt(arg('cols', '12'), 10));
const rows = Math.max(1, parseInt(arg('rows', '12'), 10));
...
const ARTKAL = (() => {
  try {
    const j = JSON.parse(readFileSync(join(process.cwd(), 'temp/artkal-palette.json'), 'utf8'));
...
/** 行字符串（level pattern 形态：色 idx→'1'..'A'，void→'.'）。 */
...
console.log(`错位打乱：${der.ok ? `全错位 OK（maxFreq=${der.maxFreq} ≤ ${Math.floor(fillN / 2)}，0 固定点）` : '不可全错位 → ' + der.reason}`);
