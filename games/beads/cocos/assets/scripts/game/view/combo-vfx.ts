 ...
 *
 * ⚠️ 平台缺口（登记台账，WXG-T-074）：Lv2 的「整屏 scale」需要**全局变换通道**，而
 * `RenderModelBuilder` 只产出逐图元命令、**没有全局变换**（`_commands` 私有）⇒ 该档的
 * **呈现**要等宿主/适配层（Canvas / Cocos）提供变换；本模块负责把**值**算对并测对（判据
 * `§8-9` 的「档位一一对应」半边），**不**在视图里假造一个"看起来像震屏"的东西顶替。
 *
 * 派生项（GDD / UX 无明文，登记台账）：
