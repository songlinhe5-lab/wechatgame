...
describe('G5 伪震屏 · 全局变换通道（art-bible §7.3.5 案 B · WXG-T-132）', () => {
    ...
    it('峰值帧：scale = 1.015、锚点 = 设计中心（=屏幕中心），恰三字段无位移', () => {
        const snap = {
            ...baseHarness('wxgame.beads.test.shake-a').game.snapshot,
            comboVfxKind: 'pseudoShake', comboVfxProgress: 0.5
        } as BeadsSnapshot;
    ...
    it('波形回 1.00（p=1）⇒ 变换位消失，不留残帧', () => {
        const snap = {
            ...baseHarness('wxgame.beads.test.shake-c').game.snapshot,
            comboVfxKind: 'pseudoShake', comboVfxProgress: 1
        } as BeadsSnapshot;
    ...
    it('D1（reduceMotion）整条关停：窗口内任意相位都无变换', () => {
        ...
        for (const p of [0.25, 0.5, 0.75]) {
            const m = modelOf({
                ...snap0, comboVfxKind: 'pseudoShake', comboVfxProgress: p,
                reduceMotion: true
            } as BeadsSnapshot);
    ...
    it('通道专属：Lv1 particles / Lv3 burst 档不触全局变换', () => {
        ...
        for (const kind of ['particles', 'burst'] as const) {
            const m = modelOf({
                ...snap0, comboVfxKind: kind, comboVfxProgress: 0.5,
                comboVfxRow: 1, comboVfxCol: 1
            } as BeadsSnapshot);
            expect('transform' in m).toBe(false);
        }
    });
    ...
    it('变换不进命令流：同 snap 仅动 comboVfx 字段 ⇒ commands 逐字节相等（图元 +0）', () => {
        ...
        const shake = modelOf({
            ...snap0, comboVfxKind: 'pseudoShake',
            comboVfxProgress: 0.5
        } as BeadsSnapshot);
