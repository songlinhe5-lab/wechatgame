...
| `hint` 提示 | 目标色底 `empty`（E1–E4 全层）+ 外描边 `accent_blue #3D7BF5` 2px + 600ms 呼吸（α 0.5↔1.0）；叠加优先级：外描边 > E2 描边 > E1 色底 `[待 ux-spec 对齐]` |
...
*本文件为资产生产与替换的权威规格。§3 网格/托盘/告急常量已按 `systems-index.md` v1.0 回写（2026-09-11）；§1.2 `empty` 行于 WXG-T-080 修订（增目标色底 E1–E4）、`locked` 行追认实现偏差（2026-09-14）；其余 `[待 ux-spec 对齐]` 动效项待 UX 规格产出后回写。*
