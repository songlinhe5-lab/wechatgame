...
2. **长期笔记**：`memory/MEMORY.md`（`.qoder/memory` → `../memory`）— 需要活上下文时 Read。

（Qoder 会自动识别根目录 `AGENTS.md`；本规则只做短指针，避免与正本双份膨胀。）

摘要（细节以正本为准）：

- 全流程 / 跨域 → `wxgame-orchestration`；单域 → `my-skills/wxgame-*`（见 `my-skills/INDEX.md`）
