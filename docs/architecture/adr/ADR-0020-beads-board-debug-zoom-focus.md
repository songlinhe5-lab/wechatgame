...
**验收判据（修复后才可执行，回应用户 (b)）**：在 `fit` 档目视/取帧确认——甲案落地后（`drawTargetTile` 边长 = `snap.gridPitch` = `BEAD_PITCH·z`，恰等于格心距）相邻底图砖**零重叠、恰好相接，缝宽恒为 0**；珠体外缘与底图外缘的间距四向对称（居中），**格间的视觉分隔线改由空坑暗缘 / 珠体倒角承担，不由底图砖缝承担**。⚠ 本条旧文原写「相邻 tile 之间出现连续的 `BEAD_GAP·z` 缝、缝的颜色 = 盘面底色（`palette.panel` 容器板 / 背景）」——那是 **tile = `cell`（乙案）** 才有的结果，非甲案；为存历史脉络原样保留并加括注：（甲案：tile = `pitch` ⇒ 缝 = 0；乙案：tile = `cell` ⇒ 留 `BEAD_GAP·z` 缝，且低倍率下缝 <1 px，未必真能露底色，见 §2.3 丙）。**在此之前，"露底色"不能作为任何参数的判据**（§2.3 丙）。
