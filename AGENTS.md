...
- **读（memory）**：日志详情**只在四种触发下查** —— ① 改冻结常量/协议/裁定**前**（查当初为什么）② 接续未完成工作 ③ 追溯**用户原话与裁定** ④ 排障找**当初的确诊法**。入口 `memory/INDEX.md`（先 grep 任务号定位到节 → 只读那一节）；`MEMORY.md` 可常读。完整协议见 `memory/INDEX.md §1`。
