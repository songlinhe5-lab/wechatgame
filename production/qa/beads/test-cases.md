...
### A4c.2 判据已冻结、`[Node]` 可证，但**本轮无落点测试** ⇒ `待执行`（**禁读作 PASS**）
...
| **F7** | 命名/口径债：`tests/denied-press.test.ts:59`、`tests/selection-anchor.test.ts:5` 将 `tapDesign()` 注释为“真链”，与 **K-038** 定义（`tapDesign` = 旁路，直调 `_handleTap`、绕开 `_readInput`）相反 | 读码（两文件均无 `input.push`） | 仅注释误称；副作用是**这些组天然不受 v2.5 时基影响（也因此不覆盖它）**，极易被误读为“已验新时基”⇒ 建议落码人改注释 + 改名或补真链双腿 |
| **F8** | 环境事实（非本单缺陷）：`node tools/scripts/sync-framework-to-cocos.mjs --check`（`framework:sync:check`）**现红** —— `beads: OUT …/cocos/assets/scripts/game/systems/board-camera.ts (differs)`；`git diff -w` 证为**工作区未提交**的 2→4 空格缩进重排（纯空白），**不属 `73d59c4`/`1bdbfd7` 入库内容**。另 `games/beads/design/ux/ux-spec.md` 有他域在途未提交改动（对应 `input-control v2.5` 变更记录“ux-spec 回写未列本单”登记） | 命令实跑 + `git show HEAD:` 对比 | 解除条件 = 落码人跑 `sync-framework` 同步镜像（提交前必达 G3）；**本单不引用为任何证据** |
