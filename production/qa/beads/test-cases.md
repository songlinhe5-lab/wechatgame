...
> - **新基线（已复算产出，可入表）**：`total 1119 = rect 487 · circle 81 · line 220 · text 9 · polygon 322`。夹具同构自证满足 K-051 前置 = 同一夹具（13×12 / 前 6 行填满 = 78 填 · 78 空 · 156 可填 / `noAssemble`）在**改码前 HEAD（`d058e5d`）**复现旧值 `1587` **逐 kind 全等** ⇒ 两侧同夹具、同序列化口径。逐 kind 差值分解与归因正本 = `assets-spec §11.2-a`（孔 `circle −78` / 刻面 kind 化 `polygon +312` / 层集退役 `rect −312`·`line −390`；非珠体族 Δ = 0，空盘 frame0 两侧 sha256 全等）。常驻判据 = `games/beads/tests/bead-style-seal.test.ts`（**进 `verify`**，⛔ 不再靠一次性手跑）；夹具 = `games/beads/tests/__fixtures__/wxg-t-211-s3-seal.json`（**夹具参数随件登记**，breakout `--frames 180` 判例）。
