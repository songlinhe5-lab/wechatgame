...
> | **TC-STY-04** | `S9 §8-17`；`§12.2 C8` + `C1`；`ux §4` 行4「生效面=下一帧」 | **下一帧生效 · 恰一帧 · B0 逐字段不变** | `[Node]` | 逐字引 §8-17 / C8：**切 styleId 后恰好一帧**内珠体族图元集发生变化、**B0 目标色底图输出逐字段不变**；钮面值翻转**无动效、零新增毫秒值**（§5 不落新行）。**三段硬断言**：① 切档**前一帧**与**后一帧**的珠体 kind 集**不相等**（差值断言，非存在性，K-060）；② 前后 B0 图元**逐字段等值**（kind/x/y/w/h/radius/fill 全列）；③ **中间帧数 = 1**（不得两帧渐变、不得零帧同帧变）。同帧图元成本变化须进 §6 差分记录（`C7`），**不得以纸面数替代实测** | 待执行（预期红基线；前置 = 步 3） |
...
>
> **K.5.1 迁移验收门（步 3 交付即跑）**：① 12 行台账逐行有落点（新文件或就地改写均须给行锚）；② 每条**作废行**的负向断言可被"复活"变异判红（K-060，逐行至少一次变异自证）；③ 新基线整帧数**由差分复算产出**并附夹具声明（须自证与 §11.2 夹具同构，否则**不得写"1587 → X"**）；④ `legacy-ten` 对照臂 = **「什么都没变」**（步 3 验收原话，SVG 逐字节等值，方法沿用 ADR-0024 J-5）。
>
