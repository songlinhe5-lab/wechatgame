...
| **面板相位真输入断链（BD-34）** | WXG-T-096 / QA v1.2 | 四面板态 `_readInput()` 无调用点；harness 可复现 ⇒ 建议立 T-100 |
| **镜像脚本缺 `.ts.meta`（BD-33）** | 同上 | `cocos:check` 不覆盖；meta 须编辑器生成 ⇒ 无编辑器即阻塞，不伪造 |
| **pre-commit 补 `sync:check` 条件步** | WXG-T-096（BD-20 复发） | 暂存含 `packages/framework/src/**` 才跑；防镜像漂移入 HEAD |
