...
> 建档：2026-09-12，主理人游承峰。当前已分配至 **WXG-T-135**（120 未启用，跳空合规），下一可用号 **WXG-T-136**。ADR-0011 = 屏幕坐标空间契约（已落盘，T-082）；ADR-0012 = G10 ES5 转译修法（T-090 待裁）。ADR-0008 编号已为「每日挑战本地确定性派生 ADR」预约（未落盘，每日挑战专项立项时使用，跳空合规）；ADR-0009 = Cocos MCP 编辑器接入（已落盘）。
