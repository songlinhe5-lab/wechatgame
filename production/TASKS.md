...
> 建档：2026-09-12，主理人游承峰。当前已分配至 **WXG-T-041**，下一可用号 **WXG-T-042**。ADR-0008 编号已为「每日挑战本地确定性派生 ADR」预约（未落盘，每日挑战专项立项时使用，跳空合规）；ADR-0009 = Cocos MCP 编辑器接入（已落盘）。
...
| WXG-T-037 | 上下文膨胀治理 R1：`ctx/reads-ledger.jsonl` 按会话分窗轮转 + 历史聚合进 `savings-baseline.json` | 程基岩 | 🔄 进行中（占位领号） | 待回填 |
