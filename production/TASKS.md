...
> 建档：2026-09-12，主理人游承峰。当前已分配至 **WXG-T-064**（进行中），下一可用号 **WXG-T-065**。ADR-0008 编号已为「每日挑战本地确定性派生 ADR」预约（未落盘，每日挑战专项立项时使用，跳空合规）；ADR-0009 = Cocos MCP 编辑器接入（已落盘）。
...
| WXG-T-064 | 台账标题制 + 详情分片（治膨胀） | 主理人(CodeBuddy) | 🔄 进行中 | 见详情 |
