...
| WXG-T-032 | ctx 索引解锁 develop：重建索引 + 登记日记文件 B 门豁免（CodeBuddy 侧会话执行） | 程基岩(CodeBuddy) | 🔄 进行中（提交 28905d5 已落；尾批 .githooks/pre-commit、AGENTS.md、tools/scripts/* 尚未提交，以该会话回报为准） | tools/scripts/{check-context-budget,install-githooks}.mjs · lib/context-index.mjs · .githooks/pre-commit · AGENTS.md |
